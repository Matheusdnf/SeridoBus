...
export default function SeridoBusApp({ navigation }: { navigation: any }) {
  ...
  const [menuVisible, setMenuVisible]   = useState(false);          // <-- controla a Sidebar
  ...
  /* ---------- efeitos ---------- */
  useEffect(()=>{ (async()=> {
    try { setCurrentUser(await UserService.current()); }
    catch(e){ console.error(e); }
  })(); },[]);

  useEffect(()=>{ (async()=> {
    try { setDestinos(await DestinationService.ListDestinations()); }
  ...
  const carregarListas = useCallback(async()=>{
    ...
    try{
      ...
      let listaI = await PassengersListService.getByDateTripShift(hoje, TripTypeEnum.Ida, turno);
      if(!listaI) listaI = await PassengersListService.create({date:hoje,trip_type:TripTypeEnum.Ida,shift:turno,company_id:1,bus_id:null});
      setListaIda(listaI.id);
      setEntriesIda(await PassengerEntryService.list(listaI.id));

      let listaV = await PassengersListService.getByDateTripShift(hoje, TripTypeEnum.Volta, turno);
      if(!listaV) listaV = await PassengersListService.create({date:hoje,trip_type:TripTypeEnum.Volta,shift:turno,company_id:1,bus_id:null});
  ...
  useEffect(()=> { if(destinos.length) carregarListas(); }, [carregarListas, destinos]);

  /* ---------- salvar passageiro ---------- */
  const nameOrUser = () => loggedIn ? userName : name.trim();
  const addToList  = async(listId:number|null)=>{
    if(!listId) return;
    await PassengerEntryService.add({
      passengers_list_id:listId,
      name: nameOrUser(),
      situation: (loggedIn?userSituation:sit) as SituationEnum,
      destination_id: destId!,
    });
    if(loggedIn) await PassengersListUsersService.add(currentUser!.id, listId);
  };
  ...
  const handleSalvar = async()=>{
    ...
    setName(''); setDestId(null); setSit('carona'); setAcao('Ida'); setViewList('ida');
  };

  /* ---------- UI ---------- */
  return (
    <SafeAreaView className="flex-1 bg-white">
      {/* Sidebar controlada por menuVisible */}
      <Sidebar
        visible={menuVisible}
      ...
      <View className="p-4">
          ...
          <View className="mb-2 px-3 py-2 border border-black rounded-xl bg-yellow-100">
            ...
            {Object.entries(
              filtrados.reduce((acc:Record<string,number>,x)=>{
                ...
                acc[nm]=(acc[nm]||0)+1; return acc;
              },{})
            ).map(([d,c])=> <Text key={d}>{d}: {c}</Text>)}
          </View>
        )}
        ...
        <View className="flex-row gap-3 mb-4">
          {(['add','ida','volta'] as const).map(k=>(
            <TouchableOpacity key={k} onPress={()=>setViewList(k)}
        ...
        {/* search */}
        {viewList!=='add' && (
          <TextInput placeholder="Pesquisar" value={searchText} onChangeText={setSearchText}
            className="border border-black p-2 rounded-md mb-4" style={{height:40,paddingLeft:10}}/>
        )}
        ...
        {viewList==='add' ? (
          <View className="gap-4">
            ...
            <Picker selectedValue={destId ?? ''}
              onValueChange={v=> setDestId(v===''?null:Number(v))}
              style={{height:50}} dropdownIconColor="black"
              ...
              {carregandoDest
                ...
                : <>
                    ...
                    {destinos.map(d=> <Picker.Item key={d.id} label={d.name} value={d.id}/>)}
                  </>
              }
            </Picker>
            ...
            <View className="flex-row justify-between">
              {(['Ida','Volta','Ida e volta'] as const).map(op=>(
                <TouchableOpacity key={op} onPress={()=>setAcao(op)}
          ...
          <FlatList
            ...
            ListHeaderComponent={()=>(
              <View className="flex-row border-b border-black bg-yellow-200 p-2">
                {['#','Nome','Destino','Situação'].map((h,i)=>(
                  <Text key={h} className={`${i===0?'w-[10%]':'w-[30%]'} font-bold text-center`}>{h}</Text>
                ))}
              </View>
            )}
            renderItem={({item,index})=>(
              <View className="flex-row border-b border-black p-2 bg-yellow-50">
                ...
                <Text className="w-[30%] text-center">{destinoNome(item.destination_id)}</Text>
                <Text className="w-[30%] text-center capitalize">{item.situation}</Text>
              </View>
            )}
